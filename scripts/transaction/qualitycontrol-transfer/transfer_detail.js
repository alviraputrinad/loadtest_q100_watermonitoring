...
export default function (data) {
  ...
  const TransferDetail_API2_res = http.get(ENDPOINTS.TRANSFER_DETAIL_API_2, {
    headers: apiHeaders, 
    tags: { name: 'TransferDetail_API2' } 
  });

  check(TransferDetail_API2_res, {
    'TransferDetail_API2: status is 200': (r) => r.status === 200,
  });

  if (TransferDetail_API2_res.status !== 200) {
      console.error(`[VU ${__VU}]: TransferDetail_API2 failed with status ${TransferDetail_API2_res.status}. Body: ${TransferDetail_API2_res.body}`);
  }
