...
export default function (data) {
  ...
  const ScheduleList_API1 = 'https://q100-api-testing-onprem.biofarma.co.id/qcs-water-monitoring/v1/Schedule/List?search=&limit=10&page=1&objectStatus=1,2,3,4&month=&year=&organizationId=239&startDate=&endDate=&startScheduleDate=&endScheduleDate=';
  const ScheduleList_API1_res = http.get(ScheduleList_API1, {
    headers: apiHeaders, 
    tags: { name: 'ScheduleList_API1' } 
  });
  ...
  if (ScheduleList_API1_res.status !== 200) {
      console.error(`[VU ${__VU}]: ScheduleList_API1 failed with status ${ScheduleList_API1_res.status}. Body: ${ScheduleList_API1_res.body}`);
  }
