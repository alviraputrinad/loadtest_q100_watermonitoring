...
export function setup() {
  ...
  const credentials = {
    userName: AUTH.OPERATOR_SAMP_WM,
    password: `P@st3ur${AUTH.OPERATOR_SAMP_WM}*()`,
...
export default function (data) {
  ...
  const SAMPLING_DETAIL_API_2_res = http.get(ENDPOINTS.SAMPLING_DETAIL_API_2, {
    headers: apiHeaders, 
    tags: { name: 'SAMPLING_DETAIL_API_2' } 
  });

  check(SAMPLING_DETAIL_API_2_res, {
    'SAMPLING_DETAIL_API_2: status is 200': (r) => r.status === 200,
  });

  if (SAMPLING_DETAIL_API_2_res.status !== 200) {
      console.error(`[VU ${__VU}]: SAMPLING_DETAIL_API_2 failed with status ${SAMPLING_DETAIL_API_2_res.status}. Body: ${SAMPLING_DETAIL_API_2_res.body}`);
  }
