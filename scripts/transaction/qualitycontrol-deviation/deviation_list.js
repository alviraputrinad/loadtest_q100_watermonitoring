...
export default function (data) {
  ...
  const IDENT_LIST_API_1_res = http.get(ENDPOINTS.IDENT_LIST_API_1, {
    headers: apiHeaders, 
    tags: { name: 'IDENT_LIST_API_1' } 
  });

  check(IDENT_LIST_API_1_res, {
    'IDENT_LIST_API_1: status is 200': (r) => r.status === 200,
  });

  if (IDENT_LIST_API_1_res.status !== 200) {
      console.error(`[VU ${__VU}]: IDENT_LIST_API_1 failed with status ${IDENT_LIST_API_1_res.status}. Body: ${IDENT_LIST_API_1_res.body}`);
  }


  const DEVIATION_LIST_API_1_res = http.get(ENDPOINTS.DEVIATION_LIST_API_1, {
    headers: apiHeaders, 
    tags: { name: 'DEVIATION_LIST_API_1' } 
  });

  check(DEVIATION_LIST_API_1_res, {
    'DEVIATION_LIST_API_1: status is 200': (r) => r.status === 200,
  });

  if (DEVIATION_LIST_API_1_res.status !== 200) {
      console.error(`[VU ${__VU}]: DEVIATION_LIST_API_1 failed with status ${DEVIATION_LIST_API_1_res.status}. Body: ${DEVIATION_LIST_API_1_res.body}`);
  }
