...
export default function (data) {
  ...
  const IDENT_LIST_API_2_res = http.get(ENDPOINTS.IDENT_LIST_API_2, {
    headers: apiHeaders, 
    tags: { name: 'IDENT_LIST_API_2' } 
  });

  check(IDENT_LIST_API_2_res, {
    'IDENT_LIST_API_2: status is 200': (r) => r.status === 200,
  });

  if (IDENT_LIST_API_2_res.status !== 200) {
      console.error(`[VU ${__VU}]: IDENT_LIST_API_2 failed with status ${IDENT_LIST_API_2_res.status}. Body: ${IDENT_LIST_API_2_res.body}`);
  }
