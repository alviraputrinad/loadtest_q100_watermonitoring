...
export default function (data) {
  ...
  const REVIEW_LIST_API_1_res = http.get(ENDPOINTS.REVIEW_LIST_API_1, {
    headers: apiHeaders, 
    tags: { name: 'REVIEW_LIST_API_1' } 
  });

  check(REVIEW_LIST_API_1_res, {
    'REVIEW_LIST_API_1: status is 200': (r) => r.status === 200,
  });

  if (REVIEW_LIST_API_1_res.status !== 200) {
      console.error(`[VU ${__VU}]: REVIEW_LIST_API_1 failed with status ${REVIEW_LIST_API_1_res.status}. Body: ${REVIEW_LIST_API_1_res.body}`);
  }
